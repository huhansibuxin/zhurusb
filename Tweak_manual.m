...
/* ---- MSHookFunction (provided by substrate/ellekit at runtime) ---- */
...
typedef void (*RBSSetForegroundProcessPID_t)(pid_t pid, int state);
static RBSSetForegroundProcessPID_t original_RBSSetForegroundProcessPID = NULL;
...
static void replaced_RBSSetForegroundProcessPID(pid_t pid, int state) {
    /* 调用原始函数 */
    if (original_RBSSetForegroundProcessPID) {
        original_RBSSetForegroundProcessPID(pid, state);
    }
...
__attribute__((constructor))
static void procguard_init(void) {
    ...
    /* Hook _RBSSetForegroundProcessPID */
    MSHookFunction((void *)_RBSSetForegroundProcessPID,
                   (void *)replaced_RBSSetForegroundProcessPID,
                   (void **)&original_RBSSetForegroundProcessPID);
...
static void procguard_cleanup(void) {
    pthread_mutex_lock(&g_lock);
    for (int i = 0; i < g_match_count; i++) free(g_match_names[i]);
    g_match_count = 0;
    pthread_mutex_unlock(&g_lock);
